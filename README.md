<div align="center">

# ⚡ SUJAL LAMICHHANE

### Cybersecurity Specialist | Certified Ethical Hacker (CEH v13)

[![Website](https://img.shields.io/badge/Website-00FFD1?style=for-the-badge&logo=google-chrome&logoColor=black)](https://sujallamichhane.com.np)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sujal-lamichhane/)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:infokaalbhairav@gmail.com)

![Profile Views](https://komarev.com/ghpvc/?username=sujallamichhane18&color=00ffd1&style=flat-square)

</div>

---

## 👨‍💻 About Me

Cybersecurity Specialist and Certified Ethical Hacker focused on building defensive security systems and conducting authorized penetration testing. I specialize in SOC operations, threat detection, and security automation using open-source tools.

**Focus Areas:**
- Security Operations Center (SOC) Architecture
- Penetration Testing & Red Team Operations
- SIEM & SOAR Implementation
- Threat Intelligence & Incident Response
- Security Automation & Tool Development

---

## 🛠️ Technical Skills

**Security Tools:**
![Kali Linux](https://img.shields.io/badge/Kali_Linux-557C94?style=flat-square&logo=kali-linux&logoColor=white)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=flat-square&logo=metasploit&logoColor=white)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=flat-square&logo=burp-suite&logoColor=white)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat-square&logo=wireshark&logoColor=white)

**SIEM & Monitoring:**
![Wazuh](https://img.shields.io/badge/Wazuh-005571?style=flat-square&logo=wazuh&logoColor=white)
![Suricata](https://img.shields.io/badge/Suricata-EF3B2D?style=flat-square&logo=suricata&logoColor=white)
![Zabbix](https://img.shields.io/badge/Zabbix-CC2936?style=flat-square&logo=zabbix&logoColor=white)

**Programming:**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)

**Platforms:**
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

---

## 🚀 Projects

### 🔐 Unified Open-Source SOC Framework
Enterprise-grade Security Operations Center built with open-source tools, featuring automated threat detection, intelligence enrichment, and incident response.

**Key Components:**
- **Network Security:** pfSense, Suricata IDS/IPS
- **SIEM & SOAR:** Wazuh, Automated Response Workflows
- **Threat Intelligence:** VirusTotal, MISP, AbuseIPDB, AlienVault OTX
- **Incident Response:** TheHive, Cortex
- **Endpoint Security:** OpenEDR, Wazuh Agent

**Features:**
- Real-time threat detection and correlation
- Automated blocking of high-severity threats (severity > 7)
- Centralized logging and alerting via Discord
- Multi-source threat intelligence integration

---

### 🛡️ Multi-Layer SIEM Security Integration
Defense-in-depth architecture with multiple security layers integrated into a centralized SIEM framework for real-time threat detection and response.

**Architecture:**
- **Layer 1:** pfSense Firewall (Perimeter Security)
- **Layer 2:** Suricata IDS/IPS (Network Detection)
- **Layer 3:** Wazuh HIDS + VirusTotal (Host Security)
- **Layer 4:** Postfix (Automated Email Alerts)

**Capabilities:**
- Cross-layer log correlation and analysis
- Real-time malicious activity detection
- Attack simulation with Kali Linux
- Automated email notifications
- File integrity monitoring and rootkit detection

---

## 📊 GitHub Stats

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=sujallamichhane18&show_icons=true&theme=radical&count_private=true&hide_border=true&bg_color=0D1117&title_color=00ffd1&text_color=ffffff&icon_color=00ffd1)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com?user=sujallamichhane18&theme=radical&hide_border=true&background=0D1117&stroke=00ffd1&ring=00ffd1&fire=00ffd1&currStreakLabel=00ffd1)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=sujallamichhane18&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=00ffd1&text_color=ffffff)

</div>

---

## 🎓 Certifications

- **Certified Ethical Hacker (CEH v13)** - EC-Council
- Security Operations & Incident Response
- Penetration Testing & Vulnerability Assessment

---

## 📫 Contact

- 🌐 Website: [sujallamichhane.com.np](https://sujallamichhane.com.np)
- 💼 LinkedIn: [Sujal Lamichhane](https://www.linkedin.com/in/sujal-lamichhane/)
- 📧 Email: infokaalbhairav@gmail.com

---

<div align="center">

### 💬 "Hack Securely. Defend Relentlessly. Learn Endlessly."

*Real hackers don't destroy systems — they reinforce them.*

---

![](https://komarev.com/ghpvc/?username=sujallamichhane18&color=00ffd1&style=flat-square&label=Profile+Views)

</div>
